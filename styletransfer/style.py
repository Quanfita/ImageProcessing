...
def get_toneMap(img, P):
    ...
    beta = beta.reshape(img.shape)
    #beta = (beta - np.min(beta)) / (np.max(beta) - np.min(beta))*5
...
def pencil_drawing(img, pencil_texture, is_color=False):
    P = cv2.cvtColor(pencil_texture,cv2.COLOR_BGR2GRAY)
    color = cv2.cvtColor(img,cv2.COLOR_BGR2YUV)
    img = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    h,w = img.shape[0],img.shape[1]
    img = cv2.resize(img,(1024,1024))
    S = get_stroke(img, 3, 8)
    # cv2.imwrite('stroke.jpg',S*255)
    T = get_toneMap(img, P)
    # cv2.imwrite('toneMap.jpg',T*255)
    res = S * T
    ...
    if is_color:
        print(color.shape)
        color[:,:,0] = res
        res = cv2.cvtColor(color, cv2.COLOR_YUV2BGR)
    # res = cv2.merge([res,res,res])
    return res

if __name__ == '__main__':
    img_path = 'lena.jpg'
    texture_path = 'pencil.jpg'
    img = cv2.imread(img_path)
    pencil_texture = cv2.imread(texture_path)
    res = pencil_drawing(img, pencil_texture, is_color=True)
    cv2.imwrite('pencil_color.png', res)
